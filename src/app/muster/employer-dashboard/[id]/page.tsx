...
export default async function MusterCandidateDetailPage({
  params,
  searchParams,
}: {
  ...
  const { employer: employerId } = await searchParams;
  ...
  // Read actual match status from DB
  ...
  if (employerId) {
    const { data: matchRow } = await supabase
      ...
      .eq("candidate_id", id)
      .eq("job_positions.employer_id", employerId)
      .limit(1)
      .maybeSingle();
  ...
  const backHref = employerId
    ? `/muster/employer-dashboard?employer=${employerId}`
    : "/muster/employer-dashboard";

  return (
    <div className="mx-auto w-full max-w-3xl space-y-6 pb-8">
      <Button variant="ghost" size="sm" asChild>
        <Link href={backHref} className="gap-2 text-muted-foreground">
